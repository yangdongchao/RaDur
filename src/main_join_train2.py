...
class Runner(object):
    ...
    def _forward(model, batch):
        inputs, frame_level_targets, slack_time, time_, embedding, embed_label, filenames, events = batch
        one_hot = np.zeros(inputs.shape[0])
        for i in range(len(events)):
            one_hot[i] = event_to_id[events[i]]
        one_hot = torch.as_tensor(one_hot).long()
        ...
        slack_time = convert_tensor(slack_time.float(), device=DEVICE, non_blocking=True)
        embedding = convert_tensor(embedding, device=DEVICE, non_blocking=True)
        one_hot = convert_tensor(one_hot, device=DEVICE,non_blocking=True)
        decision, decision_up, logit = model(inputs, embedding, frame_level_targets)
        # print('decision ',decision[0])
        # print('frame_level_targets ',frame_level_targets.shape)
        # assert 1==2
        return decision, decision_up, frame_level_targets, time_, slack_time
    ...
    def train(self, config, **kwargs):
        ...
        if config_parameters['pkl']:
            trainloader = dataset.getdataloader_join_pkl(
            config_parameters['train_data'], # feature path
            config_parameters['spk_emb_file_path'],
            transform=transform,
            batch_size=config_parameters['batch_size'],
            num_workers=config_parameters['num_workers'], time_resolution=config_parameters['time_resolution'], **sampling_kwargs)
            cvdataloader = dataset.getdataloader_join_pkl(
                config_parameters['cv_data'],
                config_parameters['spk_emb_file_path'],
                transform=None,
                shuffle=False,
                batch_size=config_parameters['batch_size'],
                num_workers=config_parameters['num_workers'], time_resolution=config_parameters['time_resolution'])
        else:
        ...
        model = getattr(models, config_parameters['model'],
                        'CRNN')(config_parameters, inputdim=64, outputdim=2,time_resolution=config_parameters['time_resolution'],
        ...
        criterion = getattr(losses, config_parameters['loss_sed'])().to(DEVICE)
        def _train_batch(_, batch):
            model.train()
            with torch.enable_grad():
                optimizer.zero_grad()
                decision, decision_up, frame_level_target, time, slack_time = self._forward(model, batch)  # output is tuple (clip, frame, target)
                loss = criterion(decision, frame_level_target, slack_time)
                #print('loss ',loss)
                # assert 1==2
                loss.backward()
                # Single loss
        ...
        def thresholded_output_transform(output):
            decision, decision_up, frame_level_target, time, slack_time = output
    ...
    def evaluate(
        ...
        #config_parameters['test_data'] = '/apdcephfs/share_1316500/donchaoyang/code2/data/additional_class_choose/strong_feature/eval_additional.h5'
        #config_parameters['label'] = '/apdcephfs/share_1316500/donchaoyang/code2/data/additional_class_choose/filelist/strong_additional_eval_neg_psd.tsv'
        # postprocessing='median'
        ...
        if config_parameters['pkl']:
            dataloader = dataset.getdataloader_join_pkl(
            config_parameters['test_data'],
            config_parameters['spk_emb_file_path'],
            batch_size=1, shuffle=False,time_resolution=config_parameters['time_resolution'])
        else:
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, unit='file', leave=False): # dataloard 加载了弱标签
                inputs, frame_level_targets, slack_time, time_, embedding,embed_label, filenames, events = batch
                decision, decision_up, frame_level_target, time,  slack_time = self._forward(model, batch) # 
